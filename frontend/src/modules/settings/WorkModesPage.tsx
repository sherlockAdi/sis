...
export default function WorkModesPage() {
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      setRows(await mastersApi.workModes.list(true));
    } catch (e: any) {
      const apiErr = e as ApiError;
      setError(apiErr?.message ?? "Failed to load work modes");
    } finally {
      setLoading(false);
    }
  }
  ...
  const cols = useMemo(
    () => [
      { field: "mode_name", headerName: "Work Mode", flex: 1, minWidth: 240 },
          ...
          return (
            <Stack direction="row" spacing={1}>
              ...
              <AdButton
                ...
                onClick={async () => {
                  await mastersApi.workModes.disable(r.work_mode_id);
                  setToast({ open: true, message: "Work mode disabled", severity: "success" });
                  refresh();
                }}
              >
                Disable
  ...
  const save = async () => {
    try {
      if (!form.mode_name.trim()) throw new Error("Work mode name is required");
      if (form.work_mode_id) {
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <AdModal
        ...
        title={form.work_mode_id ? "Edit Work Mode" : "Add Work Mode"}
        actions={
