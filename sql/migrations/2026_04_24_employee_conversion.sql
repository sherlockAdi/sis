...
BEGIN
  ...
  ELSEIF p_action = 'CREATE_FROM_DEPLOYMENT' THEN
    BEGIN
      ...
      SELECT d.deployment_id, a.candidate_id, c.passport_number, CONCAT_WS(' ', c.first_name, c.last_name),
             ...
             COALESCE(loc.city_name, loc.state_name, loc.country_name, j.job_title)
      INTO p_deployment_id, v_candidate_id, v_employee_code, v_employee_name,
           v_contact, v_address1, v_address2, v_pin_code, v_industry, v_work_location
      ...
      LEFT JOIN LOC_M03_cities loc ON loc.city_id = loc_row.city_id
      ...
      ELSE
        ...
        INSERT INTO EMP_T01_employees (
          ...
          candidate_id, deployment_id, shift_timing
        ) VALUES (
          v_employee_code, v_employee_name, v_contact,
          v_address1, v_address2, v_pin_code, v_industry, v_work_location,
          COALESCE(NULLIF(p_employment_status, ''), 'Active'), COALESCE(p_date_of_joining, CURRENT_DATE), p_date_of_confirmation,
          v_candidate_id, p_deployment_id, p_shift_timing
        );
        SET v_employee_id = LAST_INSERT_ID();

        UPDATE REC_T01_candidates
        SET status = 'Employee', updated_at = CURRENT_TIMESTAMP
        WHERE candidate_id = v_candidate_id;
  ...
  ELSEIF p_action = 'DISABLE' THEN
    ...
    SET employment_status = 'Inactive', deleted_at = CURRENT_TIMESTAMP
