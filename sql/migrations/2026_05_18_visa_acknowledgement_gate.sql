-- Visa acknowledgement gate for Step 13 / Visa Processing.
-- Adds a master checklist table and a deployment checklist status table.

CREATE TABLE IF NOT EXISTS DEP_M06_visa_checklist_master (
  checklist_item_id INT AUTO_INCREMENT PRIMARY KEY,
  checklist_item_code VARCHAR(100) NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS DEP_T06_visa_checklist_status (
  visa_checklist_status_id INT AUTO_INCREMENT PRIMARY KEY,
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uq_dep_checklist_item (deployment_id, checklist_item_id),
  FOREIGN KEY (deployment_id) REFERENCES DEP_T01_deployments(deployment_id) ON DELETE CASCADE,
  FOREIGN KEY (checklist_item_id) REFERENCES DEP_M06_visa_checklist_master(checklist_item_id) ON DELETE CASCADE,
...
ALTER TABLE DEP_T04_visa_processing_details
  DROP COLUMN IF EXISTS checklist_items_json,
  DROP COLUMN IF EXISTS checklist_complete;

INSERT INTO DEP_M06_visa_checklist_master (checklist_item_code, checklist_item_name, sort_order, is_required, status)
SELECT 'educationalCertificates', 'Educational Certificates', 10, 1, 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM DEP_M06_visa_checklist_master WHERE checklist_item_code = 'educationalCertificates'
);

INSERT INTO DEP_M06_visa_checklist_master (checklist_item_code, checklist_item_name, sort_order, is_required, status)
SELECT 'passportPhotograph', 'Passport Photograph (recent, specification-compliant)', 20, 1, 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM DEP_M06_visa_checklist_master WHERE checklist_item_code = 'passportPhotograph'
);

INSERT INTO DEP_M06_visa_checklist_master (checklist_item_code, checklist_item_name, sort_order, is_required, status)
SELECT 'medicalHealthReport', 'Medical / Health Report', 30, 1, 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM DEP_M06_visa_checklist_master WHERE checklist_item_code = 'medicalHealthReport'
);

INSERT INTO DEP_M06_visa_checklist_master (checklist_item_code, checklist_item_name, sort_order, is_required, status)
SELECT 'policeClearanceCertificate', 'Police Clearance Certificate', 40, 1, 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM DEP_M06_visa_checklist_master WHERE checklist_item_code = 'policeClearanceCertificate'
);

INSERT INTO DEP_M06_visa_checklist_master (checklist_item_code, checklist_item_name, sort_order, is_required, status)
SELECT 'notaryApprovedDocuments', 'Notary-Approved Documents', 50, 1, 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM DEP_M06_visa_checklist_master WHERE checklist_item_code = 'notaryApprovedDocuments'
);

INSERT INTO DEP_M06_visa_checklist_master (checklist_item_code, checklist_item_name, sort_order, is_required, status)
SELECT 'clientCountrySpecificRequirements', 'Any other client/country-specific requirements', 60, 1, 1
FROM DUAL
WHERE NOT EXISTS (
  SELECT 1 FROM DEP_M06_visa_checklist_master WHERE checklist_item_code = 'clientCountrySpecificRequirements'
);

DROP PROCEDURE IF EXISTS sp_dep_visa_checklist_master;
DROP PROCEDURE IF EXISTS sp_dep_visa_checklist_status;
DROP PROCEDURE IF EXISTS sp_dep_visa_details;

DELIMITER $$

CREATE PROCEDURE sp_dep_visa_checklist_master(
  IN p_action VARCHAR(30),
  IN p_checklist_item_id INT,
  IN p_checklist_item_code VARCHAR(100),
  IN p_checklist_item_name VARCHAR(255),
  IN p_sort_order INT,
  ...
  CASE UPPER(TRIM(p_action))
    WHEN 'LIST' THEN
      SELECT
        ...
        created_at,
        updated_at
      FROM DEP_M06_visa_checklist_master
      WHERE COALESCE(p_include_inactive, 0) = 1 OR status = 1
    ...
    WHEN 'GET' THEN
      ...
      LIMIT 1;
    WHEN 'CREATE' THEN
      INSERT INTO DEP_M06_visa_checklist_master (
        checklist_item_code, checklist_item_name, sort_order, is_required, status
      ) VALUES (
        p_checklist_item_code, p_checklist_item_name, COALESCE(p_sort_order, 0), COALESCE(p_is_required, 1), COALESCE(p_status, 1)
      );
      SELECT LAST_INSERT_ID() AS checklist_item_id;
    WHEN 'UPDATE' THEN
      UPDATE DEP_M06_visa_checklist_master
      SET
        checklist_item_code = COALESCE(p_checklist_item_code, checklist_item_code),
        checklist_item_name = COALESCE(p_checklist_item_name, checklist_item_name),
    ...
    WHEN 'DISABLE' THEN
      ...
      SELECT ROW_COUNT() AS affected_rows;
    WHEN 'DELETE' THEN
      DELETE FROM DEP_M06_visa_checklist_master
      WHERE checklist_item_id = p_checklist_item_id;
      SELECT ROW_COUNT() AS affected_rows;
    ELSE
      SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'sp_dep_visa_checklist_master: invalid action';
  END CASE;
END $$

CREATE PROCEDURE sp_dep_visa_checklist_status(
  IN p_action VARCHAR(30),
  IN p_deployment_id INT,
  IN p_checklist_item_id INT,
  IN p_is_checked TINYINT(1),
  IN p_user_id INT
)
BEGIN
  IF p_action = 'LIST_BY_DEPLOYMENT' THEN
    SELECT
      m.checklist_item_id,
      m.checklist_item_code,
      m.checklist_item_name,
      m.sort_order,
      m.is_required,
      COALESCE(s.is_checked, 0) AS is_checked,
      s.visa_checklist_status_id,
      s.created_at,
      s.updated_at
    FROM DEP_M06_visa_checklist_master m
    LEFT JOIN DEP_T06_visa_checklist_status s
      ON s.checklist_item_id = m.checklist_item_id
     AND s.deployment_id = p_deployment_id
    WHERE m.status = 1
    ORDER BY m.sort_order, m.checklist_item_id;

  ELSEIF p_action = 'UPSERT' THEN
    IF p_deployment_id IS NULL OR p_checklist_item_id IS NULL THEN
      SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'deployment_id and checklist_item_id are required';
    END IF;

    INSERT INTO DEP_T06_visa_checklist_status (
      deployment_id, checklist_item_id, is_checked, created_by, updated_by
    ) VALUES (
      p_deployment_id, p_checklist_item_id, COALESCE(p_is_checked, 0), p_user_id, p_user_id
    )
    ON DUPLICATE KEY UPDATE
      is_checked = COALESCE(p_is_checked, is_checked),
      updated_by = p_user_id;

    SELECT LAST_INSERT_ID() AS visa_checklist_status_id;

  ELSE
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'sp_dep_visa_checklist_status: invalid action';
  END IF;
END $$
  ...
  IF p_action = 'GET_BY_DEPLOYMENT' THEN
    SELECT
      ...
      COALESCE(
        (
          SELECT CASE
            WHEN COUNT(*) > 0 AND SUM(CASE WHEN COALESCE(s.is_checked, 0) = 1 THEN 0 ELSE 1 END) = 0 THEN 1
            ELSE 0
          END
          FROM DEP_M06_visa_checklist_master m
          LEFT JOIN DEP_T06_visa_checklist_status s
